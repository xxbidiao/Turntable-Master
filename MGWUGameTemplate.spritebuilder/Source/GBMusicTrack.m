...
- (id)initWithPath:(NSString *)path
{
    ...
    if (noErr != AudioFileOpenURL((__bridge CFURLRef)[NSURL fileURLWithPath:path], 0x01, 0, &audioFile))
    {
    ...
    AudioQueueAddPropertyListener(queue, kAudioQueueProperty_IsRunning, propertyListenerCallback, (__bridge void *)(self));
...
static void propertyListenerCallback(void *inUserData, AudioQueueRef queueObject, AudioQueuePropertyID    propertyID)
{
    ...
    if (propertyID == kAudioQueueProperty_IsRunning)
        [(__bridge GBMusicTrack *)inUserData playBackIsRunningStateChanged];
}
...
{
    // redirect back to the class to handle it there instead, so we have direct access to the instance variables
    [(__bridge GBMusicTrack *)inUserData callbackForBuffer:buffer];
}
